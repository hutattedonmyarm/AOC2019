...
std::unordered_map<Point, int> getPath(const std::string &instructions) {
    std::unordered_map<Point, int> wire{};
    std::string instruction;
    std::istringstream split(instructions);
    char direction;
    int amount, xOffset{0}, yOffset{0}, steps{0};
    Point currentPosition{0,0};
    while (std::getline(split, instruction, ',')) {
        direction = instruction[0];
        amount = std::stoi(instruction.substr(1));
        switch (direction)
        {
        case 'R':
            xOffset = 1;
        ...
        for (size_t i = 1; i <= amount; i++) {
            ...
            steps++;
            if (wire.find(currentPosition) == wire.end()) {
                wire[currentPosition] = steps;
            }
        }
    }
    return wire;
...
int main() {
    ...
    auto path1 = getPath(wire1);
    auto path2 = getPath(wire2);

    int distance = INT_MAX, length = INT_MAX;
    for (auto p1 : path1) {
        auto p2 = path2.find(p1.first);
        if (p2 != path2.end()) {
            distance = std::min(distance, abs(p1.first.x) + abs(p1.first.y));
            int totalLength = p1.second + p2->second;
            length = std::min(length, totalLength);
            std::cout << "(" << std::setw(6) << p1.first.x << "," << std::setw(6) << p1.first.y
                << ") | Wire 1: " << std::setw(6) << p1.second
                << " | Wire 2: " << std::setw(6) << p2->second
                << " | Total: " << std::setw(6) << totalLength
                << (totalLength == length ? " <-- " : "") << '\n';
        }
    }
